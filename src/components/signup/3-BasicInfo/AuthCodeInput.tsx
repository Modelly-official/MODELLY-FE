...
export const AuthCodeInput: React.FC<AuthCodeInputProps> = ({ authCode, setAuthCode, handleVerifyAuthCode, authCodeError, authCodeValid, requestSent }) => {
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      {authCodeError && (
        <p className="text-caption-1 text-error mt-1">{authCodeError}</p>
      )}
      {authCodeValid === true && (
        <p className="text-caption-1 text-blue-700 mt-1">인증번호가 확인되었습니다.</p>
      )}
    </div>
  );
};
