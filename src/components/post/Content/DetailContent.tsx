    ...
    <div className="flex flex-col gap-2 bg-gray-100 px-4 py-4">
      ...
      {detail.goal1 && (
        <div className="rounded-lg bg-white p-4">
          <InfoSection title="모델 모집 목적" content={detail.goal1} hasIcon={false} />
        </div>
      )}
      ...
      {detail.notice && (
        <div className="rounded-lg bg-white p-4">
          <InfoSection title="전달 사항" content={detail.notice} hasIcon={false} />
        </div>
      )}
      ...
      {detail.restriction && (
        <div className="flex flex-col gap-4 rounded-lg bg-white p-4">
          ...
          <div className="flex items-start gap-3 rounded-lg bg-gray-100 px-4 py-3">
            ...
            <p className="text-body-2-medium flex-1 whitespace-pre-line text-black">{detail.restriction}</p>
          </div>
        </div>
      )}
      ...
      {(detail.agreeVideo || detail.agreeInsta || detail.agreeMosaic || detail.etc) && (
        <div className="flex flex-col gap-4 rounded-lg bg-white p-4">
          ...
          {detail.etc && <InfoSection title="" content={detail.etc} hasIcon />}
        </div>
      )}
    </div>
  );
